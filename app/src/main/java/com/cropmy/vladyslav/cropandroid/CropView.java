...
/**
 * Created by vladyslav on 5/22/15.
 */
public class CropView extends ImageView {
    ...
    private static boolean isActionInsideRectangle(float x, float y) {
        int buffer = 10;
        return (x >= (leftTop.x - buffer) && x <= (rightBottom.x + buffer) && y >= (leftTop.y -
                buffer) && y <= (rightBottom.y + buffer)) ? true : false;
    }
    ...
    private boolean isInImageRange(PointF point) {
        ...
        return (point.x >= (center.x - (imageScaledWidth / 2)) && point.x <= (center.x +
                (imageScaledWidth / 2)) && point.y >= (center.y - (imageScaledHeight / 2)) &&
        ...
        switch (getAffectedSide(x, y)) {
            ...
            case DRAG:
                ...
                if (isInImageRange(new PointF(leftTop.x + movement, leftTop.y + movementY)) &&
                        isInImageRange(new PointF(rightBottom.x + movement, rightBottom.y +
                                movementY))) {
    ...
    public Bitmap getCroppedImage() {
        ...
        Bitmap cropped = Bitmap.createBitmap(drawable.getBitmap(), (int) x, (int) y, (int)
                rightBottom.x - (int) leftTop.x, (int) rightBottom.y - (int) leftTop.y);
